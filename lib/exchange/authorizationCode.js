...
module.exports = function authorizationCode(options, issue) {
  ...
  return function authorization_code(req, res, next) {
    ...
    if (!code) { return next(new AuthorizationError('Missing required parameter: code', 'invalid_request')); }
    
    issue(client, code, redirectURI, function(err, accessToken, refreshToken, params) {
      if (err) { return next(err); }
      if (!accessToken) { return next(new AuthorizationError('Invalid authorization code', 'invalid_grant')); }
