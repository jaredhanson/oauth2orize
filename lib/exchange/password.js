 ...
 * optional refresh tokens.  The callback accepts five arguments: `client`,
 * `username`, `password`, `scope` and a `done` callback, which accepts `err`,
 ...
 * authentication is performed.  `username` and `password` are the resource
 * owner's credentials, which the application must verify before issuing an
 * access token.
 *
 * Options:
 *
 ...
 *       AccessToken.create(client, username, password, function(err, accessToken) {
 ...
 *
 * References:
 *  - [Resource Owner Password Credentials Grant](http://tools.ietf.org/html/draft-ietf-oauth-v2-26#section-4.3)
 *
