...
module.exports = function(options, issue) {
  ...
  return function password(req, res, next) {
    ...
    if (!username) { return next(new AuthorizationError('Missing required parameter: username', 'invalid_request')); }
    if (!password) { return next(new AuthorizationError('Missing required parameter: password', 'invalid_request')); }
