    ...
    server._parse(type, req, function(err, areq) {
      ...
      function validated(err, client, redirectURI) {
        ...
        function immediated(err, allow, info, locals) {
          ...
          if (allow) {
            ...
            server.serializeClient(client, function(err, obj) {
              ...
              server._txnStore.store(req, req.oauth2, txn, options, function(err) {
                if (err) { return next(err); }
                next();
              });
            });
        ...
        if (arity == 4) {
          ...
          immediate(req.oauth2.client, req.oauth2.user, req.oauth2.req.scope, req.oauth2.req.audience, req.oauth2.req.type, immediated);
        } else { // arity == 3
