  ...
  return function resume(req, res, next) {
    ...
    function immediated(err, allow, info, locals) {
      ...
      if (allow) {
        ...
        if (server._txnStore.legacy == true) {
          var txn = {};
          txn.transactionID = req.oauth2.transactionID;
          txn.protocol = 'oauth2';
          ...
          server._txnStore.update(server, options, req, txn, updated);
        }
      }
    }
    ...
    if (options.passReqToCallback) {
      ...
      if (arity == 8) {
        immediate(req, req.oauth2.client, req.oauth2.user, req.oauth2.req.scope, req.oauth2.req.type, req.oauth2.req, req.oauth2.locals, immediated);
      }
    } else {
      // TODO:
    }
