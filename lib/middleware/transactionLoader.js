...
module.exports = function transactionLoader(server, options) {
  ...
  return function transactionLoader(req, res, next) {
    ...
    if (!req.session[key]) { return next(new Error('Unable load OAuth 2.0 transaction from session')); }
    ...
    server.deserializeClient(txn.client, function(err, client) {
      ...
      if (!client) {
        ...
        return next(new AuthorizationError('Client is no longer authorized', 'unauthorized_client'));
      };
