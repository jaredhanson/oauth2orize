...
module.exports = function(server, options) {
  ...
  if (server._txnStore.setOptions) {
    server._txnStore.setOptions(options);
  }
  ...
  return function transactionLoader(req, res, next) {
    ...
    // TODO: Pass `tid` in here as option
    server._txnStore.load(req, function(err, txn) {
      ...
      server.deserializeClient(txn.client, function(err, client) {
        ...
        if (!client) {
          ...
          server._txnStore.remove(req, txn.id, function(err) {
            if (err) { return next(err); }
        ...
        req.oauth2 = {};
        req.oauth2.transactionID = txn.id;
